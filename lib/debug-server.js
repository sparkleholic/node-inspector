...
function getSession(debuggerHost, debuggerPort) {
  var id = debuggerHost + debuggerPort.toString();
  var session = sessions[id];
  if (!session) {
    session = Session.create(debuggerHost, debuggerPort, config);
    sessions[id] = session;
...
function handleWebSocketConnection(socket) {
  ...
  getSession(config.debugHost, config.debugPort).join(socket);
}
